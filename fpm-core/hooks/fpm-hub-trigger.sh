...
if render_target == 'vscode':
    ...
    example_line = f"   - 예: `{save_word}. <경로>. Simple Browser POST 완료(VSCode 패널 표시). fallback URL http://jm4.local:9876{doc_route}?path=<경로>` + 핵심 요약\n"
    ...
    example_line = f"   - 예: `{save_word}. <경로>. hub URL http://jm4.local:9876{doc_route}?path=<경로>` + 핵심 요약\n"
    ...
    example_line = f"   - 예: `{save_word}. <경로>. hub URL http://jm4.local:9876{doc_route}?path=<경로> 브라우저 열림.` + 핵심 요약\n"
...
if md_first:
    ...
    step_links = (
        ...
        "`[파일명](vscode://file<파일 절대경로>)` 로 표기 (절대경로는 `/` 로 시작, 슬래시 1개. 예: `[Issue.md](vscode://file/Users/nowage/.claude/Issue.md)`). "
...
if md_first:
    ...
    step_links = (
        "4-1. **생성·수정 파일 = 클릭 링크 (Issue201)**: 본문에서 이 응답이 생성·수정·언급하는 산출물 파일 경로는 평문 나열 금지 — 반드시 `<a href=\"vscode://file<파일 절대경로>\">파일명</a>` 앵커로 렌더 (예: `<a href=\"vscode://file/Users/nowage/.claude/Issue.md\">Issue.md</a>`, 슬래시 1개). VSCode Simple Browser 에서 클릭 시 파일이 에디터로 열림 (서버 불필요).\n"
    )
