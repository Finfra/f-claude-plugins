...
#   onclick(무가드 navigator.clipboard.writeText)이 존재 — HTTP 비-localhost(jm4.local 등
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _acao(self) -> str:
        # 요청 Origin 을 반향: file://(Origin: null)·jm4.local·127.0.0.1 모두 매칭.
        # 과거 "null" 하드코딩은 jm4.local:9876 로 페이지를 열면 origin 불일치 →
...
<script>
  (function(){
    ...
    def _handle_open_prj(self, parsed):
        ...
        `ssh_remote_alias` 원격 URI 폴백을 쓴다. bind_host 를 비루프백(예: jm4.local
...
<pre>{section_html}</pre>
    ...
    def _handle_htm_doc(self, parsed):
        ...
        if abs_path not in reg_paths and rel not in reg_paths:
            ...
            #   영구 403 dead link 가 된다(2026-07-28 work-cti 실측 — 훅 정상, 트리거 부재).
...
//   HTTP 비-localhost(jm4.local 등 insecure context)에선 navigator.clipboard 미정의 → execCommand 경유.
