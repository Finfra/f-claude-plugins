...
| `--ratio-w=<N>`        | 가로 비율                         | `1.0`                   |
| `--ratio-h=<N>`        | 세로 비율                         | `1.0`                   |
| `--jpg-quality=<N>`    | JPEG 품질 (0.1~1.0)              | `0.8`                   |
| `--aspect-w=<N>`       | 이미지 종횡비 너비 (0.1~10.0)     | `1.0`                   |
| `--aspect-h=<N>`       | 이미지 종횡비 높이 (0.1~10.0)     | `1.0`                   |
| `--pdf-mode=<모드>`    | PDF 내보내기 모드                 | `firstPage`             |
| `--pdf-page=<N>`       | PDF 페이지 (selectedPage 모드용)  | `1`                     |
| `--template=<문자열>`  | 출력 파일명 템플릿                | `{name}_{rr}-{cc}`     |
...
Claude Code 내에서 실행:
```
/plugin marketplace add finfra/f-claude-plugin
/plugin install fbanner@f-claude-plugin
```

> 마켓플레이스가 `git-subdir`로 `fBanner/` 하위 경로를 자동 처리합니다.
...
| 확장                        | 위치           | 설명                                              |
| --------------------------- | -------------- | ------------------------------------------------- |
| MCP Server | fBanner 본체 레포 참조 | MCP 프로토콜로 그리드 분리 (Claude Desktop 호환)  |
