...
| --------- | ----------------------------------------- | ------ | -------------------------- |
| 상태      | `/`                                       | GET    | 헬스 체크                  |
| 스니펫    | `/api/v2/snippets`                           | GET    | 스니펫 목록(폴더 필터)     |
| 스니펫    | `/api/v2/snippets`                           | POST   | 스니펫 생성                |
| 스니펫    | `/api/v2/snippets/search`                    | GET    | 키워드로 스니펫 검색       |
| 스니펫    | `/api/v2/snippets/by-abbreviation/{abbrev}`  | GET    | 축약어로 스니펫 조회       |
| 스니펫    | `/api/v2/snippets/{id}`                      | GET    | ID로 스니펫 상세 조회      |
| 스니펫    | `/api/v2/snippets/{id}`                      | DELETE | ID로 스니펫 삭제           |
| 스니펫    | `/api/v2/snippets/expand`                    | POST   | 축약어를 텍스트로 확장     |
| 클립보드  | `/api/v2/clipboard/history`                  | GET    | 클립보드 히스토리 조회     |
| 클립보드  | `/api/v2/clipboard/history/{id}`             | GET    | 클립보드 항목 상세 조회    |
| 클립보드  | `/api/v2/clipboard/search`                   | GET    | 클립보드 히스토리 검색     |
| 폴더      | `/api/v2/folders`                            | GET    | 전체 스니펫 폴더 목록      |
| 폴더      | `/api/v2/folders`                            | POST   | 폴더 생성                  |
| 폴더      | `/api/v2/folders/{name}`                     | GET    | 폴더 상세 및 스니펫 목록   |
| 폴더      | `/api/v2/folders/{name}`                     | DELETE | 빈 폴더 삭제               |
| 통계      | `/api/v2/stats/top`                          | GET    | 상위 N 사용 통계           |
| 통계      | `/api/v2/stats/history`                      | GET    | 사용 히스토리              |
| 트리거    | `/api/v2/triggers`                           | GET    | 트리거 키 정보             |
| CLI       | `/api/v2/cli/status`                         | GET    | 엔진 실행/일시정지 상태    |
| CLI       | `/api/v2/cli/version`                        | GET    | CLI 헬퍼 버전              |
| CLI       | `/api/v2/cli/pause`                          | POST   | 스니펫 확장 일시정지       |
| CLI       | `/api/v2/cli/resume`                         | POST   | 스니펫 확장 재개           |
| CLI       | `/api/v2/reload`                             | POST   | 전체 스니펫 리소스 리로드  |
