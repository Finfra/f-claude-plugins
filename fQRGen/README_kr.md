---
title: fQRGen Claude Code Plugin (한국어)
...
| `format`     | `png` 또는 `svg`                            |
| `scale`      | QR 이미지 크기 배율, 5~20 (PNG 전용)        |
...
> 마켓플레이스가 `git-subdir`로 `fQRGen/` 하위 경로를 자동 처리합니다.
...
| 확장                  | 위치                  | 설명                                         |
| --------------------- | --------------------- | -------------------------------------------- |
| MCP Server | fQRGen 본체 레포 참조 | MCP 프로토콜로 QR 생성 (Claude Desktop 호환) |
